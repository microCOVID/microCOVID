...
Fortunately, there are now many research papers available about the numerical likelihood of getting COVID from different kinds of interactions. You can search for these studies [on Google Scholar](https://scholar.google.com/) and you can read them yourself. Some of these studies are not peer-reviewed (which is the gold standard for scientific publication), so it is extra important to analyze them carefully. We have experience interpreting scientific studies due to our academic background, and so we feel comfortable wading into the literature to come up with numerical estimates. We explain more of our reasoning in footnotes and in the Research Sources [TODO link] section.
...
For example, if you live in a region where about 1 in 1,000 people currently has COVID, then meeting a friend indoors for coffee has about a 1 in 17,000 chance of giving you COVID. Such small numbers are hard to think about, so we use microCOVIDs instead. Your coffee date would be about 60 microCOVIDs.

Another benefit of using microCOVIDs, rather than describing the chance as “1 in 17,000”,  is that you can straightforwardly add up microCOVIDs to estimate your risk over longer periods of time.[^3] For example, to use some made-up numbers to demonstrate: last week you made two trips to the grocery store (25 microCOVIDs each), went for two masked outdoor walks with friends (1 microCOVID each), and otherwise stayed home alone, so your total risk would be 52 microCOVIDs. You can imagine doing the same calculation over longer time intervals to estimate your chance of getting COVID in a month or even a year of activities.
...
[^1]:
     μ is the standard abbreviation for “micro-”, the standard unit prefix meaning “one millionth”. For example, one microgram (μg) is one millionth of a gram.
