...
Since all of the numbers in this section have some associated uncertainty, we thought it wise to attempt to estimate how much. Taking into account "known unknowns", we find that our 90% confidence interval is **about 3x in either direction** (either 3x too low or 3x too high), and that our point estimates appear unbiased relative to their distributions (although we made somewhat conservative choices in estimating the inputs to those distributions).

We've done this estimation using [Guesstimate](https://www.getguesstimate.com), an online tool that makes it easy to do math on probability distributions. (Imagine a spreadsheet where each cell is a random variable instead of just a number.) Our work is spread across two Guesstimate models:
